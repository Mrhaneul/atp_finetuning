...
per question type for ATP 2-01.3.
...
def plot(agg: dict, out_path: str, n_total: int) -> None:
    ...
    bars_base = ax.bar(x - width/2, base_vals, width,
                       label='Base Model', color='#4a90d9', alpha=0.85, zorder=3)
    bars_ft   = ax.bar(x + width/2, ft_vals,   width,
                       label='Fine-tuned (ATP)', color='#e05c5c', alpha=0.85, zorder=3)
    ...
    ax.set_title(
        'ROUGE-L: Base Model vs Fine-tuned (ATP 2-01.3)\n'
        f'{n_total} validation examples  ·  Gemma 4 E4B  ·  300 QA pairs  ·  MLX LoRA',
